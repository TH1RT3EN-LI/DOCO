...
void GridMap::initMap(rclcpp::Node::SharedPtr node)
{
  ...
  node_->declare_parameter("grid_map/cam2body_x", 0.0);
  node_->declare_parameter("grid_map/cam2body_y", 0.0);
  node_->declare_parameter("grid_map/cam2body_z", 0.0);
  ...
  node_->get_parameter("grid_map/cam2body_x", mp_.cam2body_x_);
  node_->get_parameter("grid_map/cam2body_y", mp_.cam2body_y_);
  node_->get_parameter("grid_map/cam2body_z", mp_.cam2body_z_);
  ...
  md_.proj_points_.reserve(std::max(1, 640 * 480 / mp_.skip_pixel_ / mp_.skip_pixel_));
  md_.proj_points_cnt = 0;

  md_.cam2body_ << 0.0, 0.0, 1.0, mp_.cam2body_x_,
      -1.0, 0.0, 0.0, mp_.cam2body_y_,
      0.0, -1.0, 0.0, mp_.cam2body_z_,
      0.0, 0.0, 0.0, 1.0;
  RCLCPP_INFO(node_->get_logger(),
              "grid_map cam2body default extrinsic set to t=(%.3f, %.3f, %.3f)",
              mp_.cam2body_x_, mp_.cam2body_y_, mp_.cam2body_z_);
...
void GridMap::projectDepthImage()
{
  ...
  int skip_pix = std::max(1, mp_.skip_pixel_);
  ...
  int margin = mp_.use_depth_filter_ ? std::max(0, mp_.depth_filter_margin_) : 0;
  int valid_cols = std::max(0, cols - 2 * margin);
  int valid_rows = std::max(0, rows - 2 * margin);
  size_t expected_points =
      static_cast<size_t>((valid_cols + skip_pix - 1) / skip_pix) *
      static_cast<size_t>((valid_rows + skip_pix - 1) / skip_pix);
  md_.proj_points_.reserve(expected_points);
  ...
  if (!mp_.use_depth_filter_)
  {
    for (int v = 0; v < rows; v += skip_pix)
    {
      ...
      for (int u = 0; u < cols; u += skip_pix)
      {
        ...
        depth = static_cast<double>(row_ptr[u]) / mp_.k_depth_scaling_factor_;
    ...
    else
    {
      ...
      for (int v = margin; v < rows - margin; v += skip_pix)
      {
        ...
        for (int u = margin; u < cols - margin; u += skip_pix)
        {
          ...
          depth = static_cast<double>(raw_depth) * inv_factor;
