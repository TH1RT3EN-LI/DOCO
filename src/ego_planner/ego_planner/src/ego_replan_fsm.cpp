...
namespace ego_planner
{
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    if (success)
    {
      ...
      {
        // Avoid spinning a node from inside its own callback.
        // If planner is currently generating/replanning, switch to GEN_NEW_TRAJ and let FSM consume the new target.
        changeFSMExecState(GEN_NEW_TRAJ, "TRIG");
      }
  ...
  void EGOReplanFSM::BroadcastBsplineCallback(const std::shared_ptr<const traj_utils::msg::Bspline> &msg)
  {
    ...
    planner_manager_->swarm_trajs_buf_[id].start_time_ =
        rclcpp::Time(msg->start_time, node_->get_clock()->get_clock_type());
  ...
  void EGOReplanFSM::swarmTrajsCallback(const std::shared_ptr<const traj_utils::msg::MultiBsplines> &msg)
  {
    ...
    for (size_t i = 0; i < msg->traj.size(); i++)
    {
      ...
      planner_manager_->swarm_trajs_buf_[i].start_time_ =
          rclcpp::Time(msg->traj[i].start_time, node_->get_clock()->get_clock_type());
    }
