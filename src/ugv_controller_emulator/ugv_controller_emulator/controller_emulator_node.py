...
class Duojin01ControllerEmulator(Node):
    """
    模拟车辆底盘的MCU并创建虚拟串口供仿真使用，以共用并测试ugv_base_driver包

    Data flow (driver-equivalent sim):
      ROS /ugv/cmd_vel -> ugv_base_driver(pkg) -> PTY (Duojin01 protocol) -> this node
      this node -> ROS /ugv/cmd_vel_sim -> ros_gz_bridge -> Gazebo
      Gazebo /ugv/sim/odom -> this node -> PTY feedback frames -> ugv_base_driver -> /ugv/odom + /imu
    """

    def __init__(self) -> None:
        super().__init__("ugv_controller_emulator")

        self.declare_parameter("pty_link_path", "/tmp/ugv_controller")
        self.declare_parameter("odom_topic", "/ugv/sim/odom")
        self.declare_parameter("cmd_vel_out_topic", "/ugv/cmd_vel_sim")
    ...
    def _make_pty_link(self) -> None:
        ...
        self._rx_thread = threading.Thread(target=self._rx_loop, name="ugv_pty_rx", daemon=True)
